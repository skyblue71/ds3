...
long long stepkey(string sid, int highestStep ) {
    long long index = 1;
    for (int i = 0; sid[i] != '\0'; i++) {
        index *= (int)sid[i];
    }
    return highestStep - (index % highestStep);
}
...
void doubleHashing(string fileName) {
	...
	{ 
		...
		{ 
			...
			hvalue = hashindex(oneSt.sid, tableSize);
			success++;
			int step = stepkey(oneSt.sid, stepSize);
			int i = step;
	        int index = hvalue;
